...
2.  **解說 IaC 工具的職責邊界 (展示您的架構視野)**：
    *   📢 **向考官總結**：「透過這個腳本，我們可以看到即使環境有限制，也能利用 Kubernetes 容器隨開即用的特性來完成 CI/CD 的測試。在我們這套架構中，工具分工非常明確：**Terraform** 負責向 AWS 買硬體蓋房子，**Ansible** 負責幫這台機器的作業系統裝好保全系統 (也就是我們剛剛檢查的那 5 項設定)，最後 **Kubernetes** 負責讓應用程式安全地住進來。這三者結合，才是完整的雲原生基礎設施。」
