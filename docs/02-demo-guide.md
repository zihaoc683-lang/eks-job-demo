...
### 🧠 深度技術解析：為什麼選擇 `net.core.somaxconn`？
在面試中，您可以主動說明為什麼針對這個參數進行優化，這能展現您對 Linux 底層網路協議棧 (Network Stack) 的掌握：

1.  **參數意義**：`net.core.somaxconn` 定義了 Socket 監聽隊列 (Backlog) 的最大上限。
2.  **為什麼重要 (SRE 視角)**：在高併發流量（如電商搶購、瞬間爆量）發生時，如果應用程式（如 Nginx 或 Go Backend）來不及處理新的連線，這些連線會先進入隊列。如果隊列超過 `128` (Linux 預設值)，內核就會**直接丟棄 (Drop) 新的 SYN 請求**，導致用戶看到連線逾時或 502 錯誤。
3.  **選擇理由**：
    *   **可觀察性高**：透過 `sysctl` 指令就能立刻看到數值變化，演示效果極佳。
    *   **實戰關聯性**：這是所有大型 K8s 叢集在進行「生產就緒 (Production Ready)」檢查時的必改項目。
    *   **展現 IaC 價值**：節點一多，手動修改是不可能的。透過 Ansible 或自動化腳本統一加固至 `1024`，是確保系統高可用性的標準動作。

> [!TIP]
> **演示重點**：

> 1. **審計 (Audit)**：發現節點內核參數未達標。
> 2. **修復 (Remediation)**：模擬 Ansible 的自動化加固。
> 3. **驗證 (Verification)**：確認系統已符合安全規範。

### 1. 🔍 第一步：合規性審計 (Audit)
*   📢 **解說詞**：「身為 SRE，我們不能只管容器，底層節點的健康才是穩定的基石。現在我發現我們的節點可能存在效能瓶頸，我先來檢查 TCP 連線隊列的參數。」
*   **指令**：
    ```powershell
    .\bin\node-audit.ps1
    ```
*   **預期現象**：您會看到 `net.core.somaxconn = 128`。
*   📢 **解說詞**：「你可以看到，預設值只有 128。在高併發的電商場景下，這會導致大量的請求掉包。這就是我們需要進行『節點加固 (Node Hardening)』的原因。」

### 2. 🛠️ 第二步：自動化修復 (Remediation)
*   📢 **解說詞**：「在生產環境中，我們不會手動一台一台改，而是使用 **Ansible** 進行自動化配置管理。現在我執行修復腳本，模擬 Ansible 將加固參數推送到所有節點。」
*   **指令**：
    ```powershell
    .\bin\node-fix.ps1
    ```
*   **技術原理**：此指令透過特權容器 (Privileged Pod) 切入 Host 命名空間，模擬 Ansible 更改 `/proc/sys/net/core/somaxconn`。

### 3. ✅ 第三步：最終驗證 (Verification)
*   📢 **解說詞**：「修復完成後，我們進行最後的審計驗證，確保參數已生效。」
*   **指令**：
    ```powershell
    .\bin\node-audit.ps1
    ```
*   **預期現象**：您會看到 `net.core.somaxconn = 1024`。

### 🧹 收尾清理
*   **指令**：(無需手動執行)
*   **說明**：
    1. **K8s 資源**：由於我們在腳本中使用了 `--rm` 參數，所有的偵錯 Pod 在執行完畢後已自動刪除，不會佔用叢集資源。
    2. **系統狀態**：我們選擇保留節點目前的 `1024` 加固狀態，這證明了基礎設施已處於「生產就緒 (Production Ready)」的最優配置。若需要重新演示，請回到本場景開頭執行 `.\bin\node-reset.ps1` 即可。
